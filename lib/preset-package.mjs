...
export function validatePresetPackage(document) {
  ...
  if (document.tools !== undefined) {
    if (!isRecord(document.tools)) throw new Error('预设包 tools 必须为对象');
    for (const key of ['presets', 'groups']) {
      if (document.tools[key] !== undefined && !Array.isArray(document.tools[key])) {
...
export function encodePresetPackage(record, state) {
  const document = record.sharePackage ? structuredClone(record.sharePackage) : {
    ...
    tools: { version: 1, activePresetId: null, presets: [], groups: [] },
    extensions: {},
  };
