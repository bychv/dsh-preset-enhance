...
function validatePrefill(prefill) {
    if (prefill === null || prefill === undefined)
        return;
    if (!isRecord(prefill))
        throw new Error('预设包 prefill 必须为对象或 null');
    for (const key of ['enabled', 'toolCalls', 'removeNonOfficialTools']) {
        if (typeof prefill[key] !== 'boolean')
            throw new Error(`预设包 prefill.${key} 必须为布尔值`);
    }
...
export function validateToolsPackage(tools) {
    if (tools === undefined)
        return validateToolsSection(tools);
    if (!isRecord(tools))
...
export function validatePresetPackage(document) {
    if (!isRecord(document) || document.format !== PRESET_PACKAGE_FORMAT)
        throw new Error('无法识别预设包格式');
    if (document.version !== PRESET_PACKAGE_VERSION)
        throw new Error('不支持的预设包版本，请升级插件后重试');
    ...
    if (document.metadata !== undefined && !isRecord(document.metadata))
        throw new Error('预设包 metadata 必须为对象');
    if (document.metadata?.name !== undefined && typeof document.metadata.name !== 'string')
        throw new Error('预设包名称必须为文本');
    validatePrefill(document.prefill);
    if (document.tools !== undefined)
        validateToolsPackage(document.tools);
    if (document.extensions !== undefined && !isRecord(document.extensions))
        throw new Error('预设包 extensions 必须为对象');
    return document;
}
export function decodePresetDocument(document, fallbackName = '未命名预设') {
    if (isRecord(document) && (document.format === PRESET_PACKAGE_FORMAT || !Array.isArray(document.prompts))) {
        ...
        return {
            ...
            preset: structuredClone(container.data),
