...
export function validatePreset(preset) {
  ...
  for (const p of preset.prompts) {
    ...
    if (p.injection_depth != null && (!Number.isInteger(p.injection_depth) || p.injection_depth < 0)) throw new Error('injection_depth 必须是非负整数');
...
export function compilePreset(preset, history = [], options = {}) {
  ...
  if (afterTool && typeof options.postToolPrefix === 'string' && options.postToolPrefix.trim() &&
    ...
    if (text.trim()) {
      tail = { ...tail, content: [{ type: 'text', text }] };
