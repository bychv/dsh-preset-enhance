...
export function validatePreset(preset) {
  if (!preset || !Array.isArray(preset.prompts) || preset.prompts.length > 2000) throw new Error('预设必须包含 prompts 数组（最多 2000 项）');
  const ids = new Set();
  for (const p of preset.prompts) {
    if (!p || typeof p.identifier !== 'string' || !p.identifier || ids.has(p.identifier)) throw new Error('提示词 identifier 缺失或重复');
    ids.add(p.identifier);
    if (p.content != null && typeof p.content !== 'string') throw new Error('content 必须是文本');
    if (p.role != null && !['system', 'user', 'assistant', 'model'].includes(p.role)) throw new Error(`不支持的角色：${p.role}`);
    if (p.injection_position != null && ![0, 1].includes(p.injection_position)) throw new Error('injection_position 必须是 0 或 1');
    for (const key of ['injection_depth', 'injection_order']) if (p[key] != null && (!Number.isInteger(p[key]) || p[key] < 0)) throw new Error(`${key} 必须是非负整数`);
  ...
  for (const group of preset.prompt_order ?? []) {
    if (!group || !Array.isArray(group.order)) throw new Error('顺序表缺少 order 数组');
    const seen = new Set();
    for (const p of group.order) {
      if (!p || typeof p.identifier !== 'string' || seen.has(p.identifier) || typeof p.enabled !== 'boolean') throw new Error('顺序表条目无效或重复');
...
export function compilePreset(preset, history = [], options = {}) {
  ...
  for (const item of getOrder(preset, options.characterId)) {
    ...
    if (p.injection_trigger?.length && !p.injection_trigger.includes(options.trigger ?? 'normal')) continue;
  ...
  if (preset.assistant_prefill) {
    ...
    ctx.warnings.push('assistant_prefill 作为尾部 assistant 消息发送；是否续写由模型适配器决定');
  }
  return { messages, entries, local: { ...ctx.local }, global: { ...ctx.global }, warnings: [...new Set(ctx.warnings)] };
}
