...
import { createOutputExtractionStream, extractOutputText, extractTaggedOutputFallback, stripExtractionSpecialTokens, } from './output-extractor.mjs';
const D = '｜｜DSML｜｜';
...
const ESCAPED_MARKER_RE = new RegExp(String.fromCharCode(92, 92) + "(?=<" + PAD_SOURCE + OPTIONAL_SLASH + PAD_SOURCE +
    "(?:tool" + PAD_SOURCE + "_?" + PAD_SOURCE + "calls?|(?:invoke|parameter)|" + PIPE_SOURCE + "(?:DSML|invoke|parameter)" + PAD_SOURCE + "))", "giu");
...
const officialMarker = (word) => PIPE_SOURCE + 'tool' + SEP_SOURCE + 'calls' + SEP_SOURCE + word + PAD_SOURCE + PIPE_SOURCE;
...
function sniffBase64Mime(value) {
    for (const [magic, mime] of B64_MAGIC)
...
function harvestImages(value, images) {
    if (Array.isArray(value))
        return value.map(item => harvestImages(item, images));
    if (typeof value === 'string')
        return harvestString(value, images);
    ...
    for (const [key, item] of Object.entries(value)) {
        if (key === 'b64_json' && typeof item === 'string' && item.length > 200) {
            ...
            output[key] = harvestImages(item, images);
    }
    return output;
}
function toolResultContent(content) {
    ...
    if (typeof content === 'string') {
        ...
        if (compact.startsWith('{') || compact.startsWith('[')) {
            try {
                cleaned = harvestImages(JSON.parse(compact), images);
            }
            catch {
                cleaned = harvestString(content, images);
            }
        }
        else
            cleaned = harvestString(content, images);
        ...
        cleaned = harvestImages(content, images);
...
function qualifiedToolDefinitionName(tool) {
...
export function buildToolsPrompt(tools) {
    ...
    lines.push('To call tools, use EXACTLY this DSML format:', DSML_CALLS_OPEN, '<' + D + ' invoke name="tool_name">', '<' + D + ' parameter name="param_name" string="true">string value</' + D + ' parameter>', '<' + D + ' parameter name="param_name" string="false">123</' + D + ' parameter>', '</' + D + ' invoke>', DSML_CALLS_CLOSE, '', 'Rules:', '- string="true" for string values (write the raw text, no extra quotes)', '- string="false" for numbers, booleans, null, arrays and objects (write valid JSON)', '- Multiple invokes may be chained inside one calls block', '- tool_name must exactly match one of the tools above', '- Do not wrap the DSML calls block in a Markdown code fence');
    return lines.join('\n');
}
...
function qualifiedToolName(toolCall) {
    ...
    while (index < messages.length) {
        ...
        if (message?.role === 'tool') {
            ...
            while (index < messages.length && messages[index]?.role === 'tool') {
                ...
                texts.push('<tool_execution_result tool_call_id="' + id + '" tool_name="' + name + '">\n' +
 ...
 */
function parseInvokeCalls(segment, patterns) {
    const calls = [];
    ...
    for (let index = 0; index < invokes.length; index += 1) {
        ...
        for (let parameterIndex = 0; parameterIndex < parameters.length; parameterIndex += 1) {
            ...
            if (stringFlag === "true")
                args[parameterName] = selfClosing || parameterClose ? raw : raw.trimEnd();
            else {
                try {
                    args[parameterName] = JSON.parse(raw.trim());
                }
                catch {
                    args[parameterName] = raw.trim();
                }
            }
        }
...
function parseOfficialCalls(segment) {
    return parseInvokeCalls(normalizeDsmlText(segment), OFFICIAL_PATTERNS);
}
function normalizeDsmlText(value) {
    return String(value ?? "")
...
function matchAfter(pattern, text, start) {
    pattern.lastIndex = 0;
...
function findToolCallsBegin(text) {
    ...
    if (begin)
        candidates.push({ kind: "wrapper", match: begin });
    const plain = firstMatch(TOOL_CALLS_BEGIN_RE, text);
    if (plain)
        candidates.push({ kind: "wrapper", match: plain });
    ...
    if (official)
        candidates.push({ kind: "official", match: official });
    const invoke = firstMatch(DSML_INVOKE_OPEN_RE, text);
    if (invoke)
        candidates.push({ kind: "invoke", match: invoke });
...
function isPartialToolMarker(value) {
...
export function transformToolCallJson(data, metadata) {
    ...
    output.choices = Array.isArray(data.choices) ? data.choices.map((choice) => {
        ...
        if (metadata.extractOutput === true && typeof message.reasoning_content === 'string') {
            ...
            if (contentEmpty && fallback.matched)
    ...
    transform(choice, forceFinal = false) {
        ...
        if (typeof delta.reasoning_content === 'string') {
            if (this.extractor) {
                ...
                if (reasoningVisible)
                    delta.reasoning_content = reasoningVisible;
                else
