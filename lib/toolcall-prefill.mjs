...
function qualifiedToolDefinitionName(tool) {
  ...
  const rawNamespace = tool?.namespace ?? fn.namespace;
...
function qualifiedToolName(toolCall) {
  ...
  const name = String(fn.name ?? '');
  if (name.includes('::')) return name;
  const namespace = toolCall?.namespace ?? fn.namespace;
  return namespace == null || String(namespace).trim() === '' ? name : String(namespace).trim() + '::' + name;
}
...
function findToolCallsBegin(text) {
  const candidates = [
    ['wrapper', firstMatch(DSML_BEGIN_RE, text)],
    ['wrapper', firstMatch(TOOL_CALLS_BEGIN_RE, text)],
    ['invoke', firstMatch(DSML_INVOKE_OPEN_RE, text)],
  ].filter(([, match]) => match);
  if (candidates.length === 0) return null;
  const [kind, match] = candidates.reduce((best, item) => item[1].index < best[1].index ? item : best);
  const index = match.index > 0 && text[match.index - 1] === '\\' ? match.index - 1 : match.index;
  return { kind, index, text: match[0] };
}
