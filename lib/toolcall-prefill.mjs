...
class ChoiceState {
    ...
    if (final) {
      ...
      if (this.nativeReasoningBuffer) {
        ...
        if (!this.bodyObserved && fallback.matched) {
          extractedReasoning += fallback.reasoning;
          ...
          if (parsedReasoning.toolCalls) {
            extractedReasoning += stripExtractionSpecialTokens(parsedReasoning.content);
            this.reasoningToolCalls = mergeToolCalls(this.reasoningToolCalls, parsedReasoning.toolCalls);
          } else {
            extractedReasoning += stripExtractionSpecialTokens(this.nativeReasoningBuffer);
          }
        }
        this.nativeReasoningBuffer = '';
      }
  ...
  drainRaw() {
    ...
    reasoning += stripExtractionSpecialTokens(this.nativeReasoningBuffer);
    this.nativeReasoningBuffer = '';
