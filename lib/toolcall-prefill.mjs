...
export function parseDsmlCalls(segment) {
    ...
    for (let index = 0; index < invokes.length; index += 1) {
        ...
        const bodyEnd = Math.min(nextInvoke, invokeClose?.index ?? Number.POSITIVE_INFINITY, dsmlClose?.index ?? Number.POSITIVE_INFINITY, toolClose?.index ?? Number.POSITIVE_INFINITY);
