...
export function normalizeToolPresetRules(value) {
  ...
  for (const raw of list) {
    ...
    if (typeof raw.enabled !== 'boolean') throw new Error(`工具预设规则 ${toolRefLabel(ref)} 的 enabled 必须是布尔值`);
    const key = toolRefKey(ref.modeId, ref.toolName);
...
export function normalizeToolState(state) {
  const groups = Array.isArray(state.toolGroups) ? state.toolGroups : [];
  state.toolGroups = groups
    .filter(group => isRecord(group) && typeof group.id === 'string' && group.id.length > 0)
    .map(group => ({
      id: group.id,
      ...
      members: (Array.isArray(group.members) ? group.members : [])
        .filter(member => isRecord(member) && typeof member.modeId === 'string' && member.modeId.length > 0 &&
          typeof member.toolName === 'string' && member.toolName.length > 0)
        .map(member => ({ modeId: member.modeId, toolName: member.toolName })),
    }));
  const presets = Array.isArray(state.toolPresets) ? state.toolPresets : [];
  state.toolPresets = presets
    .filter(preset => isRecord(preset) && typeof preset.id === 'string' && preset.id.length > 0)
    .map(preset => ({
      id: preset.id,
      ...
      rules: (Array.isArray(preset.rules) ? preset.rules : [])
        .filter(rule => isRecord(rule) && typeof rule.modeId === 'string' && typeof rule.toolName === 'string')
  ...
  state.modeToolSelections = isRecord(state.modeToolSelections) ? state.modeToolSelections : {};
  state.sessionToolSelections = isRecord(state.sessionToolSelections) ? state.sessionToolSelections : {};
  ...
  for (const [modeId] of entries(state.modeToolPolicies)) {
    if (!isRecord(ownGet(state.modeToolSelections, modeId))) assign(state.modeToolSelections, modeId, { kind: 'custom' });
  }
  for (const [sessionId] of entries(state.sessionToolPolicies)) {
    if (!isRecord(ownGet(state.sessionToolSelections, sessionId))) assign(state.sessionToolSelections, sessionId, { kind: 'custom' });
  }
  for (const [modeId, selection] of entries(state.modeToolSelections)) {
    ...
    if (!valid) assign(state.modeToolSelections, modeId, { kind: 'custom' });
  }
  for (const [sessionId, selection] of entries(state.sessionToolSelections)) {
    const valid = isRecord(selection) && (selection.kind === 'inherit' || selection.kind === 'custom' ||
      (selection.kind === 'preset' && typeof selection.presetId === 'string' && presetIds.has(selection.presetId)));
    if (!valid) assign(state.sessionToolSelections, sessionId, { kind: 'inherit' });
  }
  return state;
}
...
export function toolPolicySnapshot(state) {
  ...
  const modes = {};
  for (const [modeId, policy] of entries(state?.modeToolPolicies)) if (isRecord(policy)) assign(modes, modeId, { ...policy });
  const sessions = {};
  for (const [sessionId, policy] of entries(state?.sessionToolPolicies)) if (isRecord(policy)) assign(sessions, sessionId, { ...policy });
  const modeSelections = {};
  for (const [modeId, selection] of entries(state?.modeToolSelections)) if (isRecord(selection)) assign(modeSelections, modeId, { ...selection });
  const sessionSelections = {};
  for (const [sessionId, selection] of entries(state?.sessionToolSelections)) if (isRecord(selection)) assign(sessionSelections, sessionId, { ...selection });
...
/**
 * The single resolver behind both the request tool schema and the execution guard.
...
export function effectiveToolEnabled(snapshot, sessionId, modeId, toolName) {
  const inherited = modeDecision(snapshot, modeId, toolName);
...
export function presetReferenceCounts(state) {
  const counts = {};
  const ensure = id => {
    if (!ownGet(counts, id)) assign(counts, id, { modes: 0, sessions: 0 });
    return ownGet(counts, id);
  };
  for (const preset of Array.isArray(state?.toolPresets) ? state.toolPresets : []) {
    if (typeof preset?.id === 'string' && preset.id.length > 0) ensure(preset.id);
  }
  for (const selection of Object.values(state?.modeToolSelections ?? {})) {
    if (selection?.kind === 'preset' && typeof selection.presetId === 'string') ensure(selection.presetId).modes++;
  }
  for (const selection of Object.values(state?.sessionToolSelections ?? {})) {
    if (selection?.kind === 'preset' && typeof selection.presetId === 'string') ensure(selection.presetId).sessions++;
  }
  return counts;
}
...
export function unresolvedToolRefs(state, catalogs) {
  ...
  const consider = (ref, kind, owner) => {
    if (typeof ref?.modeId !== 'string' || typeof ref?.toolName !== 'string') return;
  ...
  for (const group of Array.isArray(state?.toolGroups) ? state.toolGroups : []) {
    for (const member of Array.isArray(group?.members) ? group.members : []) consider(member, 'group', group);
  }
  for (const preset of Array.isArray(state?.toolPresets) ? state.toolPresets : []) {
    for (const rule of Array.isArray(preset?.rules) ? preset.rules : []) consider(rule, 'preset', preset);
  }
  return refs;
}
...
export function validateToolsSection(tools) {
  ...
  if (!Number.isInteger(version) || version < 1) throw new Error('预设包 tools.version 必须是正整数');
  if (version !== 1) return { version, applicable: false, activePresetId: null, presets: [], groups: [] };
