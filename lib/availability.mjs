 ...
 */
export function markPresetEnhanceActive() {
    const store = slots();
    const slot = store[MARKER] ?? { count: 0 };
    slot.count += 1;
    store[MARKER] = slot;
    let released = false;
    return () => {
...
export function isPresetEnhanceActive() {
    return (slots()[MARKER]?.count ?? 0) > 0;
}
/**
 ...
 */
export function setPresetEnhanceUnavailableReason(reason) {
    slots()[UNAVAILABLE_REASON] = reason;
}
/** Forget a previous startup failure once the plugin starts cleanly again. */
export function clearPresetEnhanceUnavailableReason() {
    delete slots()[UNAVAILABLE_REASON];
}
/** Generic reason used when the plugin is simply not enabled. */
export const PRESET_ENHANCE_INACTIVE_REASON = '预设增强插件未启用：st-preset 模式只负责预设注入与工具策略，插件关闭时不会注入任何内容。'
...
export function presetEnhanceUnavailableReason() {
