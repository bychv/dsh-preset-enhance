 ...
 */
export function readPromptRegexOptions(preset) {
...
export function writePromptRegexOptions(preset, patch) {
    const extensions = { ...(isRecord(preset.extensions) ? preset.extensions : {}) };
    const namespace = { ...(isRecord(extensions[PRESET_EXTENSION_KEY]) ? extensions[PRESET_EXTENSION_KEY] : {}) };
...
export function readRegexScripts(preset) {
    const extensions = preset?.extensions;
    const list = isRecord(extensions) ? extensions.regex_scripts : undefined;
    if (!Array.isArray(list))
        return [];
    return list.filter(isRecord).slice(0, SCRIPT_LIMIT);
}
/** Fill missing or conflicting rule ids once; an existing id is never regenerated. */
export function ensureRegexIds(scripts, mint = index => 'regex-' + (index + 1)) {
    const seen = new Set();
...
export function regexPlacement(raw) {
    const values = (Array.isArray(raw) ? raw : [raw]).filter((value) => typeof value === 'number' && Number.isFinite(value));
    const targets = [];
    const unsupported = [];
    for (const value of values) {
        if (value === PLACEMENT_USER_INPUT) {
            if (!targets.includes('user'))
                targets.push('user');
        }
        else if (value === PLACEMENT_AI_OUTPUT) {
            if (!targets.includes('assistant'))
...
export function depthInWindow(script, depth) {
...
export function regexName(script) {
 ...
 */
export function planRegexScript(script, depth) {
    ...
    if (script.disabled === true)
        return { ...base, runs: false, supported: true, reason: '已停用' };
    if (!find)
        return { ...base, runs: false, supported: false, reason: '查找式为空' };
...
function sanitizeFlags(raw) {
    ...
    let out = '';
    for (const ch of raw)
        if (ALLOWED_FLAGS.includes(ch) && !seen.has(ch)) {
            seen.add(ch);
            out += ch;
        }
    return out;
...
function isPatternUsable(pattern, substitute) {
    try {
        compilePattern(expandPattern(pattern, substitute, null));
        return true;
    }
    catch {
        return false;
...
function substituteMode(raw) {
    if (typeof raw === 'number' && Number.isFinite(raw))
        return raw === SUBSTITUTE_ESCAPED ? SUBSTITUTE_ESCAPED : raw === SUBSTITUTE_RAW ? SUBSTITUTE_RAW : 0;
    const name = typeof raw === 'string' ? raw.trim().toUpperCase() : '';
    if (name === 'ESCAPED')
        return SUBSTITUTE_ESCAPED;
...
function expandPattern(pattern, substitute, ctx) {
    const how = substituteMode(substitute);
    if (how === 0 || ctx === null)
        return pattern;
    if (how === SUBSTITUTE_RAW)
        return renderMacros(pattern, ctx);
...
function resolveTrimStrings(raw, ctx) {
...
function filterLiterals(value, trim) {
    let out = value;
    for (const literal of trim)
        if (literal)
 ...
 */
export function renderReplacement(template, match, trim) {
    let out = '', cursor = 0;
    while (cursor < template.length) {
        if (template.startsWith('{{match}}', cursor)) {
            out += filterLiterals(match.full, trim);
            cursor += 9;
            continue;
        }
        ...
        if (named) {
            out += filterLiterals(match.named[named[1]] ?? '', trim);
...
export function runRegexScriptDetailed(text, script, ctx) {
    const find = typeof script.findRegex === 'string' ? script.findRegex : '';
    if (script.disabled === true || !find || text === '')
        return { text, matched: false, replacements: 0 };
    ...
    try {
        ...
        throw new Error('正则规则「' + regexName(script) + '」的查找式无效：' + message(error));
    }
    ...
    let matched = false;
    let replacements = 0;
    const replaced = text.replace(regex, (...args) => { matched = true; replacements += 1; return renderReplacement(template, matchOf(args), trim); });
...
export function runRegexScript(text, script, ctx) {
 ...
 */
export function applyPromptRegex(text, scripts, ctx, options) {
    let out = text;
    const applied = [];
    for (const script of scripts) {
        const plan = planRegexScript(script, options.depth);
        if (!plan.runs || !plan.targets.includes(options.target))
            continue;
 ...
 */
export function chatTargetOf(message) {
    if (message.role !== 'user' && message.role !== 'assistant')
        return undefined;
    if (typeof message.source?.kind === 'string' && NON_CHAT_SOURCES.has(message.source.kind))
        return undefined;
    const blocks = message.content ?? [];
    if (blocks.some(block => block.type === 'tool-result'))
        return undefined;
    if (!blocks.some(block => block.type === 'text' && (block.text ?? '').length > 0))
        return undefined;
 ...
 */
export function buildChatDepths(history) {
    const depths = new Map();
    let depth = -1;
    let previousWasAssistant = false;
    for (let index = history.length - 1; index >= 0; index--) {
        const message = history[index];
        if (message === undefined || chatTargetOf(message) === undefined)
            continue;
 ...
 */
export function planChatMessage(message, target, depth) {
    const segments = [];
    const pieces = [];
    let buffer = [];
    const flush = () => {
...
export function applyChatSegments(plan, texts) {
    const content = plan.pieces.map(piece => 'block' in piece ? piece.block : { type: 'text', text: texts[piece.run] ?? '' });
 ...
 */
export function rewriteChatText(message, scripts, ctx, options) {
