/** Left-to-right, balanced macro evaluation. Values are data, never eval'ed. */
export function createMacroContext(options = {}) {
  return {
    ...
    values: Object.fromEntries(Object.entries(options.values ?? {}).map(([k, v]) => [k.toLowerCase(), String(v)])),
...
export function renderMacros(input, ctx = createMacroContext(), level = 0) {
  if (level > 32) throw new Error('宏嵌套超过 32 层');
  if (typeof input !== 'string' || input.length > 2_000_000) throw new Error('提示词必须是小于 2 MB 的文本');
  ...
  while (cursor < input.length) {
    ...
    if (input[cursor] === '\\' && '{}'.includes(input[cursor + 1] ?? '\0')) {
      out += input[cursor + 1]; cursor += 2; continue;
    }
    if (input.slice(cursor, cursor + 2) !== '{{') {
      const ch = input[cursor++];
      if (trimNext && (ch === '\r' || ch === '\n')) continue;
      trimNext = false; out += ch; continue;
    }
    const start = cursor;
    let depth = 1; cursor += 2;
    const bodyStart = cursor;
    while (cursor < input.length && depth) {
      if (input[cursor] === '\\') { cursor += 2; continue; }
      const pair = input.slice(cursor, cursor + 2);
      if (pair === '{{') { depth++; cursor += 2; }
      else if (pair === '}}') { depth--; if (depth) cursor += 2; }
      else cursor++;
    }
    if (depth) { ctx.warnings.push('未闭合的宏'); out += input.slice(start); break; }
    const raw = input.slice(bodyStart, cursor); cursor += 2;
    if (--ctx.remaining < 0) throw new Error('宏执行数量超过 10000');
    if (raw.trimStart().startsWith('//')) continue; // Nested side effects in comments never execute.
    if (raw.trim().toLowerCase() === 'trim') { out = out.replace(/[\r\n]+$/, ''); trimNext = true; continue; }
    ...
    const args = parts.map(p => renderMacros(p, ctx, level + 1).trim());
    ...
    if (variable) {
      ...
      if (op === 'set' && args.length === 1 && args[0].includes(':')) {
        const at = args[0].indexOf(':'); args.splice(0, 1, args[0].slice(0, at), args[0].slice(at + 1));
      }
      const key = args[0];
      if (!key) { ctx.warnings.push(`${name} 缺少变量名`); continue; }
      const current = vars[key] ?? '';
      if (op === 'set') vars[key] = args.slice(1).join('::');
      if (op === 'get') value = current;
      if (op === 'flush') delete vars[key];
      if (op === 'inc' || op === 'dec') value = vars[key] = String((Number(current) || 0) + (op === 'inc' ? 1 : -1));
      if (op === 'add') {
      ...
      if (!dice || Number(dice[1] || 1) > 100 || Number(dice[2]) < 1 || Number(dice[2]) > 1e9) {
        ctx.warnings.push('无效骰子表达式'); value = input.slice(start, cursor);
      } else {
        let sum = Number(dice[3] ?? 0);
        for (let i = 0; i < Number(dice[1] || 1); i++) sum += 1 + Math.floor(ctx.random() * Number(dice[2]));
        value = String(sum);
      }
...
function splitArguments(raw) {
  ...
  for (let i = 0; i < raw.length - 1; i++) {
    ...
    else if (!depth && pair === '::') { parts.push(raw.slice(start, i)); start = i + 2; i++; }
  }
  parts.push(raw.slice(start)); return parts;
}
