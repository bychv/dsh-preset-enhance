...
export const CONTROL_TAGS = [OUTPUT_OPEN, OUTPUT_CLOSE, LEGACY_OUTPUT_OPEN, LEGACY_OUTPUT_CLOSE];
...
const EXTRACTION_SPECIAL_TOKENS = [END_MARKER, LEGACY_END_MARKER].concat(CONTROL_TAGS);
...
const HOLD_PATTERNS = [
  END_MARKER, LEGACY_END_MARKER, THINK_OPEN, CONTENT_OPEN, CONTENT_CLOSE, ANCHOR_SENTENCE,
...
export function createStrategyScanner(options) {
  const config = Object.assign({}, STRATEGY_DEFAULTS, options || {});
  const outputTags = [OUTPUT_OPEN, LEGACY_OUTPUT_OPEN];
  const closeTags = [OUTPUT_CLOSE, LEGACY_OUTPUT_CLOSE];
...
export function compareStrategies(text) {
  ...
  const signals = [END_MARKER, LEGACY_END_MARKER, '</think>', '</thinking>']
