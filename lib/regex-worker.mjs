...
function handle(message) {
    ...
    let rule = '';
    try {
        const ctx = createMacroContext({ local: task.local, global: task.global, values: task.values, seed: task.seed });
        ...
        for (let index = 0; index < task.draws; index++)
            ctx.random();
        ...
        for (let ruleIndex = 0; ruleIndex < task.scripts.length; ruleIndex++) {
            const script = task.scripts[ruleIndex];
            ...
            for (let index = 0; index < task.segments.length; index++) {
                ...
                if (!plan.runs || !plan.targets.includes(segment.target))
                    continue;
                ...
                if (replacements > task.limits.maxReplacements)
                    throw new Error('替换次数超过上限 ' + task.limits.maxReplacements);
                outputChars += result.text.length - before.length;
                if (outputChars > task.limits.maxOutputChars)
                    throw new Error('正则处理后文本超过 ' + task.limits.maxOutputChars + ' 字符');
                texts[index] = result.text;
                if (!applied.includes(rule))
                    applied.push(rule);
            }
        }
