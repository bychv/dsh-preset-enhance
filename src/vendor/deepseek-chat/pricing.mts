...
import type { ChatConnectionConfig, ChatModelConfig } from './config.mjs';
import type { ImageAttachmentAccess, ImageAttachmentRef, ImageBlock, LlmImageRequestPrice, LlmImageRequestPricing } from './host-types.mjs';
...
export interface ProjectedDimensions { width: number; height: number }
...
export function resolveRequestImageMaxBytes(model: ChatModelConfig): number {
...
export function resolveRequestImageTarget(
  ...
  source: Pick<ImageAttachmentRef, 'width' | 'height'> & { width: number; height: number },
): ProjectedDimensions & { maxBytes: number } {
...
export function deepSeekImageRequestPricing(
  ...
  model: string,
  resolveAccess?: (ref: ImageAttachmentRef) => ImageAttachmentAccess | undefined,
  ...
  if (catalogModel?.inputModalities?.includes('image') !== true) {
    return { priceImages: images => images.map((block): LlmImageRequestPrice => ({ visualTokens: 0, text: textOnlyImageText(block.attachment) })) };
  }
  return {
    priceImages: images => images.map((block): LlmImageRequestPrice => {
      const ref = block.attachment;
      if (block.offloaded === true) return { visualTokens: 0, text: offloadedImageText(ref, resolveAccess?.(ref)) };
