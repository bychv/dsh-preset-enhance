...
import type {
  PresetPackageDocument, PresetPackagePrefill, PresetRecord, PresetState, SillyTavernPreset, ToolsSection,
} from './types.mjs';
...
const emptyToolsSection = (): ToolsSection => ({ version: 1, activePresetId: null, presets: [], groups: [] });

function validatePrefill(prefill: unknown): void {
  ...
  if (prefill.extractOutput !== undefined && typeof prefill.extractOutput !== 'boolean') {
    throw new Error('预设包 prefill.extractOutput 必须为布尔值');
  }
  const post = prefill.postToolPrefix;
  if (!isRecord(post) || !['inherit', 'custom'].includes(post.mode as string) || typeof post.text !== 'string') {
 ...
 */
export function validateToolsPackage(tools: unknown): ToolsSection {
  if (tools === undefined) return validateToolsSection(tools) as ToolsSection;
  if (!isRecord(tools)) throw new Error('预设包 tools 必须为对象');
  const section = validateToolsSection(tools) as ToolsSection;
...
export function validatePresetPackage(document: unknown): PresetPackageDocument {
  ...
  return document as PresetPackageDocument;
}

export function decodePresetDocument(document: unknown, fallbackName = '未命名预设'):
{ name: string; preset: SillyTavernPreset; sharePackage?: PresetPackageDocument } {
  if (isRecord(document) && (document.format === PRESET_PACKAGE_FORMAT || !Array.isArray(document.prompts))) {
    validatePresetPackage(document);
    const container = isRecord(document.preset) ? document.preset : {};
    const metadata = isRecord(document.metadata) ? document.metadata : {};
    return {
      name: typeof metadata.name === 'string' && metadata.name ? metadata.name : fallbackName,
      preset: structuredClone(container.data) as SillyTavernPreset,
      sharePackage: structuredClone(document) as PresetPackageDocument,
    };
  }
  validatePreset(document);
  return { name: fallbackName, preset: structuredClone(document) as SillyTavernPreset };
}

export function prefillFromState(state: PresetState): PresetPackagePrefill {
  return {
 ...
 */
export function encodePresetPackage(record: PresetRecord, state: PresetState, tools?: unknown): PresetPackageDocument {
  const document: PresetPackageDocument = record.sharePackage ? structuredClone(record.sharePackage) : {
  ...
  if (tools !== undefined && tools !== null) {
    ...
    document.tools = structuredClone(tools) as ToolsSection;
  }
...
export function attachPrefillSettings(record: PresetRecord, state: PresetState): void {
...
export function applyPackagePrefill(state: PresetState, record: PresetRecord): void {
  ...
  state.prefixOutputExtraction = prefill.extractOutput === true;
