/**
 * Pure Anthropic Messages <-> OpenAI Chat Completions translators.
 *
 * DSH 0.1.6 defaults the official DeepSeek connection to the Messages
 * protocol (POST https://api.deepseek.com/anthropic/v1/messages, x-api-key
 * plus anthropic-version). Preset-enhance chat mode reroutes such a request to
 * the official Chat Completions endpoint, so this module owns the whole wire
 * translation:
 *
 *  - request body: Messages -> chat/completions ({@link messagesRequestToChat})
 *  - request headers: x-api-key -> Authorization: Bearer, Anthropic-only
 *    headers dropped ({@link messagesRequestHeadersToChat})
 *  - streaming response: chat/completions SSE -> Messages SSE, incremental and
 *    never buffering a whole stream ({@link ChatToMessagesStream},
 *    {@link createChatToMessagesStream}, {@link translateChatResponse})
 *  - non-stream response: chat completion -> Anthropic message object
 *    ({@link chatCompletionToMessage})
 *
 * The module has no runtime dependencies and no module-level state: every
 * function is pure or owns an explicit instance. Every field without a faithful
 * Chat Completions representation is reported through {@link TranslationNote}
 * instead of being dropped silently.
 *
 * Verified against the pinned host source
 * (packages/llm/llm-deepseek/src/protocols/{messages,chat-completions}).
 */
import type { JsonObject } from './toolcall-prefill.mjs';

/** One field this translation could not carry over, reported instead of dropped. */
export interface TranslationNote {
  /** Wire field or block type that was not translated. */
  field: string;
  /** What happened to it, in the language the workbench renders. */
  detail: string;
}
...
export interface MessagesRequestTranslation {
  ...
  body: JsonObject;
  /** Everything that had no faithful chat/completions representation. */
  notes: TranslationNote[];
}

/** Anthropic stop_reason vocabulary the harness Messages adapter accepts. */
export type AnthropicStopReason = 'end_turn' | 'stop_sequence' | 'tool_use' | 'max_tokens';

export interface ChatToMessagesOptions {
  /** Model id used for message_start when the upstream chunk omits one. */
  model?: string;
  /** Clock for synthesised message ids; injectable for deterministic tests. */
  now?: () => number;
}
...
class Notes {
  private readonly items: TranslationNote[] = [];
  private readonly seen = new Set<string>();

  add(field: string, detail: string): void {
...
function joinText(blocks: readonly unknown[]): string {
  let out = '';
  for (const block of blocks) if (isObject(block) && block.type === 'text') out += text(block.text);
  return out;
}

function noteCacheControl(block: JsonObject, note: (field: string, detail: string) => void): void {
  if (block.cache_control !== undefined) {
    note('cache_control', '缓存提示是 Anthropic 专有字段，Chat Completions 无对应能力，已丢弃。');
  }
}

function noteUnsupportedBlock(block: JsonObject, role: string, note: (field: string, detail: string) => void): void {
  note(role + ' block ' + String(block.type), 'Chat Completions 没有对应内容类型，已丢弃。');
}

/* --------------------------------------------------------------- request side */

function systemText(system: unknown, note: (field: string, detail: string) => void): string {
  if (typeof system === 'string') return system;
  if (!Array.isArray(system)) {
    if (system != null) note('system', '无法识别的 system 形态，已丢弃。');
    return '';
  }
  let out = '';
  for (const raw of system) {
    if (!isObject(raw)) { note('system[]', '非对象 system 块已丢弃。'); continue; }
    if (raw.type === 'text') { out += text(raw.text); noteCacheControl(raw, note); continue; }
    noteUnsupportedBlock(raw, 'system', note);
  }
  return out;
}

function toolResultText(content: unknown, note: (field: string, detail: string) => void): string {
  if (typeof content === 'string') return content;
  if (!Array.isArray(content)) return content == null ? '' : text(content);
  let out = '';
...
function imagePart(block: JsonObject, note: (field: string, detail: string) => void): JsonObject | null {
  ...
  if (source.type === 'file') return { type: 'file', file_id: text(source.file_id) };
...
function userParts(blocks: readonly JsonObject[], note: (field: string, detail: string) => void): JsonObject[] {
  const parts: JsonObject[] = [];
  for (const block of blocks) {
    if (block.type === 'text') {
      if (text(block.text).length > 0) parts.push({ type: 'text', text: text(block.text) });
...
function assistantMessage(blocks: readonly JsonObject[], note: (field: string, detail: string) => void): JsonObject {
  let body = '';
  let reasoning = '';
  const toolCalls: JsonObject[] = [];
  for (const block of blocks) {
    if (block.type === 'text') { body += text(block.text); noteCacheControl(block, note); continue; }
  ...
  const message: JsonObject = { role: 'assistant', content: body };
  if (reasoning.length > 0) message.reasoning_content = reasoning;
  if (toolCalls.length > 0) message.tool_calls = toolCalls;
  return message;
}

function appendMessage(raw: unknown, out: JsonObject[], note: (field: string, detail: string) => void): void {
  if (!isObject(raw)) { note('messages[]', '非对象消息已丢弃。'); return; }
  const role = raw.role;
  const blocks = contentBlocks(raw.content);
  if (role === 'system') {
    out.push({ role: 'system', content: joinText(blocks) });
    for (const block of blocks) if (isObject(block) && block.type !== 'text') noteUnsupportedBlock(block, 'system', note);
    return;
  }
  if (role === 'assistant') {
  ...
  if (role !== 'user') note('messages[].role=' + String(role), '未知角色按 user 处理。');
  const toolResults: JsonObject[] = [];
  const regular: JsonObject[] = [];
  for (const block of blocks) {
...
function toolsOf(raw: readonly unknown[], note: (field: string, detail: string) => void): JsonObject[] {
  const tools: JsonObject[] = [];
  for (const tool of raw) {
    if (!isObject(tool)) { note('tools[]', '非对象工具定义已丢弃。'); continue; }
 ...
 */
export function mapToolChoice(choice: unknown, note: (field: string, detail: string) => void): JsonObject | undefined {
  if (!isObject(choice)) return undefined;
  let mapped: JsonObject | undefined;
  if (choice.type === 'auto') mapped = { type: 'auto' };
  else if (choice.type === 'none') mapped = { type: 'none' };
  else if (choice.type === 'any') mapped = { type: 'required' };
  else if (choice.type === 'tool') mapped = { type: 'function', function: { name: text(choice.name) } };
  else {
    note('tool_choice.type=' + String(choice.type), 'Chat Completions 没有对应 tool_choice，已丢弃。');
    return undefined;
  }
  if (choice.disable_parallel_tool_use === true) mapped.parallel_tool_calls = false;
  return mapped;
}
 ...
 */
export function messagesRequestToChat(body: JsonObject): MessagesRequestTranslation {
  const notes = new Notes();
  const note = (field: string, detail: string): void => notes.add(field, detail);
  const chat: JsonObject = {};

  if (body.model !== undefined) chat.model = body.model;
  if (body.stream === false) chat.stream = false;
  else {
    chat.stream = true;
  ...
  if (body.top_k !== undefined) note('top_k', 'Chat Completions 没有 top_k 字段，已丢弃。');
  if (Array.isArray(body.stop_sequences)) chat.stop = [...body.stop_sequences];
  else if (body.stop_sequences !== undefined) note('stop_sequences', '非数组 stop_sequences 已丢弃。');
  ...
  if (effort === 'low' || effort === 'high' || effort === 'max') chat.reasoning_effort = effort;
  ...
  if (isObject(body.output_config)) {
    for (const key of Object.keys(body.output_config)) {
      if (key !== 'effort') note('output_config.' + key, 'Chat Completions 没有对应字段，已丢弃。');
    }
  }
  ...
  } else if (body.tools !== undefined) note('tools', '非数组 tools 已丢弃。');
  ...
  if (Array.isArray(body.messages)) {
    for (const message of body.messages) appendMessage(message, messages, note);
  } else if (body.messages !== undefined) note('messages', '非数组 messages 已丢弃。');
  chat.messages = messages;

  for (const key of ['metadata', 'container', 'mcp_servers', 'service_tier']) {
    if (body[key] !== undefined) note(key, 'Anthropic 专有请求字段在 Chat Completions 上没有对应能力，已丢弃。');
  }
...
function headerEntries(headers: unknown): Array<[string, string]> {
  ...
  if (Array.isArray(headers)) {
    ...
    for (const item of headers) {
      if (Array.isArray(item)) out.push([String(item[0]), String(item[1] ?? '')]);
    }
    return out;
  }
  if (typeof headers === 'object' || typeof headers === 'function') {
    ...
    if (typeof entries === 'function') {
      const out: Array<[string, string]> = [];
      for (const entry of (entries as () => Iterable<[string, string]>).call(headers)) {
 ...
 */
export function messagesRequestHeadersToChat(headers: unknown): Record<string, string> {
  const out: Record<string, string> = {};
  let apiKey = '';
  for (const [name, value] of headerEntries(headers)) {
    const lower = name.toLowerCase();
    if (lower === 'x-api-key') { apiKey = value; continue; }
    if (lower === 'anthropic-version' || lower === 'anthropic-beta' || lower === 'content-length') continue;
    out[lower] = value;
  }
  if (apiKey.length > 0 && out.authorization === undefined) out.authorization = 'Bearer ' + apiKey;
 ...
 */
export function mapChatFinishReason(reason: unknown): AnthropicStopReason {
 ...
 */
function applyUsage(target: AnthropicUsage, usage: JsonObject): void {
  ...
  if (cacheRead !== undefined && cacheRead > 0) target.cache_read_input_tokens = cacheRead;
}

function usagePayload(usage: AnthropicUsage): JsonObject {
  const payload: JsonObject = { input_tokens: usage.input_tokens, output_tokens: usage.output_tokens };
  if (usage.cache_read_input_tokens !== undefined) payload.cache_read_input_tokens = usage.cache_read_input_tokens;
  return payload;
}

function sseData(raw: string): string | null {
  const values: string[] = [];
...
interface OpenBlock {
  /** Anthropic wire index, assigned in first-appearance order. */
  index: number;
  ...
  id?: string;
  name?: string;
  /** Argument JSON accumulated so far (empty until the first fragment). */
  argumentsText: string;
  /** Fragments buffered before the tool identity (id + name) is known. */
  pending: string;
}
...
export class ChatToMessagesStream {
  private readonly options: ChatToMessagesOptions;
  private readonly blocks = new Map<string, OpenBlock>();
  private readonly usage: AnthropicUsage = { input_tokens: 0, output_tokens: 0 };
  private buffer = '';
  private started = false;
  private done = false;
  private nextIndex = 0;
  private reason: AnthropicStopReason | null = null;
  private model = 'unknown';
  private id = '';

  constructor(options: ChatToMessagesOptions = {}) {
  ...
  finish(): string {
    ...
    if (this.buffer.length > 0) {
      ...
      if (this.done) return frame;
      return frame + this.closeWithoutTerminal();
    }
    if (this.reason !== null) return this.finalize();
    return this.closeWithoutTerminal();
  }

  private frame(type: string, payload: JsonObject): string {
    return 'event: ' + type + '\ndata: ' + JSON.stringify(payload) + '\n\n';
  }

  private handleFrame(raw: string): string {
    const data = sseData(raw);
    if (data === null) return '';
    if (data.trim() === '[DONE]') return this.finalize();
    let chunk: unknown;
    try {
  ...
  private ensureStarted(chunk: JsonObject): string {
  ...
  private consume(chunk: JsonObject): string {
    ...
    if (choice) {
      ...
      if (typeof choice.finish_reason === 'string') this.reason = mapChatFinishReason(choice.finish_reason);
    }
    if (isObject(chunk.usage)) applyUsage(this.usage, chunk.usage);
    return out;
  }

  private consumeDelta(delta: JsonObject): string {
    let out = '';
    ...
    if (Array.isArray(delta.tool_calls)) {
      for (const call of delta.tool_calls) out += this.consumeToolDelta(call);
    }
    return out;
  }

  private openTextBlock(kind: 'text' | 'thinking'): string {
    if (this.blocks.has(kind)) return '';
    const state: OpenBlock = { index: this.nextIndex++, kind, started: true, open: true, argumentsText: '', pending: '' };
  ...
  private block(kind: 'text' | 'thinking'): OpenBlock {
    return this.blocks.get(kind) as OpenBlock;
  }

  private deltaFrame(kind: 'text' | 'thinking', deltaType: string, delta: JsonObject): string {
  ...
  private consumeToolDelta(raw: unknown): string {
    ...
    if (typeof raw.id === 'string' && raw.id.length > 0) state.id = raw.id;
    const fn = isObject(raw.function) ? raw.function : {};
    if (typeof fn.name === 'string' && fn.name.length > 0) state.name = fn.name;
    if (typeof fn.arguments === 'string' && fn.arguments.length > 0) state.pending += fn.arguments;

    let out = '';
    if (!state.started && state.id !== undefined && state.name !== undefined) out += this.startToolBlock(state);
    if (state.started && state.pending.length > 0) out += this.argumentsFrame(state, state.pending);
    return out;
  }

  private startToolBlock(state: OpenBlock): string {
    state.started = true;
    return this.frame('content_block_start', {
      type: 'content_block_start',
      index: state.index,
  ...
  private argumentsFrame(state: OpenBlock, partial: string): string {
  ...
  private openToolBlocks(): string {
    let out = '';
    for (const state of this.blocks.values()) {
      if (state.kind !== 'tool' || state.started) continue;
      out += this.startToolBlock(state);
      if (state.pending.length > 0) out += this.argumentsFrame(state, state.pending);
    }
    return out;
  }

  private closeBlocks(): string {
    let out = this.openToolBlocks();
    for (const state of this.blocks.values()) {
      if (!state.open) continue;
      // The host rejects an empty tool argument string on a settled response.
      if (state.kind === 'tool' && state.argumentsText.length === 0) out += this.argumentsFrame(state, '{}');
      state.open = false;
  ...
  private closeWithoutTerminal(): string {
    if (this.done) return '';
    this.done = true;
...
export function createChatToMessagesStream(options: ChatToMessagesOptions = {}): TransformStream<Uint8Array, Uint8Array> {
...
function messageId(data: JsonObject, options: ChatToMessagesOptions): string {
 ...
 */
export function chatCompletionToMessage(data: JsonObject, options: ChatToMessagesOptions = {}): JsonObject {
  const choice = Array.isArray(data.choices) && isObject(data.choices[0]) ? data.choices[0] : {};
  const message = isObject(choice.message) ? choice.message : {};
  const content: JsonObject[] = [];
  const reasoning = message.reasoning_content;
  if (typeof reasoning === 'string' && reasoning.length > 0) content.push({ type: 'thinking', thinking: reasoning });
  const body = message.content;
  if (typeof body === 'string' && body.length > 0) content.push({ type: 'text', text: body });
  ...
  if (isObject(data.usage)) applyUsage(usage, data.usage);
  return {
 ...
 * the original bytes.
 */
export async function translateChatResponse(response: Response, options: ChatToMessagesOptions = {}): Promise<Response> {
