...
export function stripExtractionSpecialTokens(text: unknown): string {
  let output = typeof text === 'string' ? text : '';
  for (const token of EXTRACTION_SPECIAL_TOKENS) output = output.split(token).join('');
  return output;
}
 ...
 */
export function extractTaggedOutputFallback(reasoning: unknown): TaggedOutputFallback {
  ...
  for (const [open, close] of pairs) {
    ...
    for (;;) {
      ...
      if (closeAt < 0) break;
      if (!best || openAt >= best.openAt) best = { open, close, openAt, closeAt };
  ...
  const content = stripExtractionSpecialTokens(
    source.slice(best.openAt + best.open.length, best.closeAt));
  if (!content.trim()) {
    return { matched: false, reasoning: stripExtractionSpecialTokens(source), content: '' };
  }
...
/** Tunables of the end-of-thinking / output-region strategy. */
export interface StrategyConfig {
  ...
  anchorLineStart: boolean;
  stripControlTags: boolean;
  /** Region gate for </think>; only honoured by the scanner. */
  gateCloseThink?: boolean;
  /** Alias of contentFallback the scanner accepts under its signal name. */
  contentTrigger?: boolean;
}

export type StrategyOptions = Partial<StrategyConfig>;

/** One scanner event; `text` is '' for structural (output-open/close) events. */
...
export interface EarliestMatch { index: number; text: string }
...
export interface StrategyScanner {
  readonly phase: string;
  readonly switchedAt: number;
  readonly switchMode: string | null;
  ...
  push(chunk: unknown): ScannerEvent[];
  finish(): ScannerEvent[];
}

export interface TaggedOutputFallback { matched: boolean; reasoning: string; content: string }
...
export interface OutputExtractionStream {
  readonly phase: string;
  readonly switched: boolean;
  readonly switchMode: string | null;
  assumeBody(): { reasoning: string; content: string };
  push(chunk: unknown): { reasoning: string; content: string };
  finish(): { reasoning: string; content: string };
}
...
export interface StrategyMeasure { switchedAt: number; leak: number; miss: boolean; late: number }

export interface StrategyComparison {
  bodyStart: number;
  firstMatch: StrategyMeasure;
  strategy: StrategyMeasure & { restatements: number; fallbackUsed: boolean; mode: string | null };
  safeStrategy: StrategyMeasure & { restatements: number; fallbackUsed: boolean; mode: string | null };
}
...
export function lookaheadCharsFor(options?: StrategyOptions): number {
...
export function stripControlTags(text: unknown): string {
  let output = typeof text === 'string' ? text : '';
  for (const tag of CONTROL_TAGS) output = output.split(tag).join('');
  return output;
}

function textKindForPhase(phase: string): string {
...
function findEarliest(haystack: string, needles: readonly string[]): EarliestMatch | null {
  let best: EarliestMatch | null = null;
  for (const needle of needles) {
    const index = haystack.indexOf(needle);
    if (index < 0) continue;
    if (best === null || index < best.index) best = { index, text: needle };
  }
  return best;
}

/** 找“独占一行”的规范结束标记。 */
export function findStandaloneMarker(buffer: string, atEnd: boolean, atLineStart: boolean): MarkerCandidate | null {
  let from = 0;
  for (;;) {
    ...
    if (startsLine && endsLine) {
      ...
      if (atEnd) return { markerIndex: index, markerEnd: afterIndex, lineEnd: afterIndex, lineComplete: false };
 ...
 */
function isLineStart(buffer: string, index: number, atLineStart: boolean): boolean {
  const lineBreak = buffer.lastIndexOf('\n', index - 1);
  if (lineBreak >= 0) return buffer.slice(lineBreak + 1, index).trim() === '';
 ...
 */
function buildPrefixSet(patterns: readonly string[]): Set<string> {
  const set = new Set<string>();
  for (const pattern of patterns) {
    for (let length = 1; length < pattern.length; length += 1) set.add(pattern.slice(0, length));
  }
  return set;
}
...
function holdLength(buffer: string, prefixes: ReadonlySet<string>, holdLimit: number): number {
  const limit = Math.min(buffer.length, holdLimit - 1);
  for (let length = limit; length > 0; length -= 1) {
    if (prefixes.has(buffer.slice(buffer.length - length))) return length;
  }
  return 0;
}
 ...
 */
export function createStrategyScanner(options?: StrategyOptions): StrategyScanner {
  const config = Object.assign({}, STRATEGY_DEFAULTS, options || {});
  ...
  let emitted = 0;
  let switchedAt = -1;
  let switchMode: string | null = null;
  let restatements = 0;
  ...
  function emitText(events: ScannerEvent[], text: string): void {
    if (!text) return;
    events.push({ kind: textKindForPhase(phase), text });
    consume(text);
  }

  function holdAndFlush(events: ScannerEvent[], prefixes: ReadonlySet<string>): void {
  ...
  function drain(atEnd: boolean): ScannerEvent[] {
    const events = [];
    for (;;) {
      if (phase !== 'thinking') {
      ...
      if (candidate && (!close || candidate.markerIndex <= close.index)) {
        ...
        let earliest = -1;
        let confirmed = null;
        const consider = (at: number, kind: string): void => {
          if (at < 0) return;
          if (earliest < 0 || at < earliest) { earliest = at; confirmed = kind; }
        };
...
export function scanWithStrategy(text: unknown, chunkSize?: number, options?: StrategyOptions): ScanResult {
  const config = Object.assign({}, STRATEGY_DEFAULTS, options || {});
  ...
  for (let index = 0; index < source.length; index += size) {
    for (const event of scanner.push(source.slice(index, index + size))) events.push(event);
  }
  for (const event of scanner.finish()) events.push(event);
  for (const event of events) {
    if (event.kind === 'restatement' || event.kind === 'output-open' || event.kind === 'output-close') continue;
    output += event.text;
  }
 ...
 */
export function compareStrategies(text: unknown): StrategyComparison {
  const source = typeof text === 'string' ? text : '';
  ...
  const measure = (switchedAt: number): StrategyMeasure => {
    if (switchedAt < 0) return { switchedAt: -1, leak: 0, miss: true, late: 0 };
    if (bodyStart < 0) return { switchedAt, leak: 0, miss: false, late: 0 };
...
function extractionParts(events: ScannerEvent[]): { reasoning: string; content: string } {
  let reasoning = '';
  let content = '';
  for (const event of events) {
    if (event.kind === 'thinking') reasoning += event.text;
    else if (event.kind === 'body' || event.kind === 'tool-call' || event.kind === 'text') content += event.text;
  }
  return {
 ...
 */
export function extractOutputText(text: unknown, { assumeBody = false }: { assumeBody?: boolean } = {}): ExtractionResult {
  const source = typeof text === 'string' ? text : '';
  const scanner = createStrategyScanner({ gateCloseThink: true, outputTrigger: true });
  const events = [];
  if (assumeBody) events.push(...scanner.push(OUTPUT_OPEN));
...
export function createOutputExtractionStream({ assumeBody = false }: { assumeBody?: boolean } = {}): OutputExtractionStream {
  const scanner = createStrategyScanner({ gateCloseThink: true, outputTrigger: true });
  let forced = false;
  function forceBody() {
    if (forced || scanner.phase !== 'thinking') return { reasoning: '', content: '' };
    forced = true;
    return extractionParts(scanner.push(OUTPUT_OPEN));
  }
  if (assumeBody) forceBody();
  return {
    ...
    push(chunk: unknown) { return extractionParts(scanner.push(chunk)); },
