...
import type { ToolSchemaRow } from '../host-types.mjs';
...
/** One child plugin row of a preset composition. */
export interface ModePluginRow {
  id?: string;
  name: string;
  config?: unknown;
  group?: boolean | null;
...
/** A preset definition as the registry's `register()` takes it. */
export interface ModeDefinition {
  id: string;
  name?: string;
  description?: string;
  order?: number;
  plugins: readonly ModePluginRow[];
}

/** The scope lease `acquireScope()` hands out. */
export interface ModeScopeLease {
  /** Opaque host scope identity; pass it to `tools.schemas()`. */
  readonly key: unknown;
  [Symbol.asyncDispose](): Promise<void>;
}

/** The 0.1.7 registry surface this module uses. */
export interface ModePresetServiceLike {
  register?(definition: ModeDefinition): Promise<() => Promise<void>>;
  acquireScope?(id?: string): Promise<ModeScopeLease>;
}

/** One loader entry, read structurally (`Entry` from the Cordis loader). */
export interface LoaderEntryLike {
  readonly id?: string;
  readonly options?: { id?: string; name?: string; config?: unknown };
}

/** The loader surface this module uses. */
export interface ModeLoaderLike {
  entries(): Iterable<LoaderEntryLike>;
}

/** The tools surface this module uses. */
export interface ModeToolsLike {
  schemas?(scope?: unknown): ToolSchemaRow[];
}

/** One live agent, read structurally. */
export interface LiveAgentLike {
  readonly ctx?: { readonly tools?: ModeToolsLike };
}

/**
 * The host surface this module needs. Every member is optional and structural,
 * so the plugin's own `PluginContext` satisfies it without a cast even while
 * its declaration still carries the 0.1.6 shapes.
 */
export interface ModeHostLike {
  readonly loader?: ModeLoaderLike;
  readonly agentPresets?: ModePresetServiceLike;
  readonly tools?: ModeToolsLike;
  readonly agents?: { get(id: string): LiveAgentLike | undefined };
}

/** What the running host can do for declarative mode registration. */
export interface ModeCapability {
  /** `agentPresets.register()` is available. */
  declarative: boolean;
  /** `agentPresets.acquireScope()` is available for catalog reads. */
  scopeLease: boolean;
  /** The 0.1.6 directory surface is present instead. */
  legacyDirectory: boolean;
  /** Empty when both 0.1.7 capabilities exist, otherwise a user-facing reason. */
  reason: string;
}

/** One resolved `standard` declaration plus the entry it came from. */
export interface StandardModeDeclaration {
  /** Loader entry id of the declaring row (e.g. `preset-standard`). */
  entryId: string;
  /** The complete declaration, including its full `plugins` list. */
...
function isPluginRow(value: unknown): value is ModePluginRow {
 ...
 */
export function modeCapability(host: ModeHostLike | undefined): ModeCapability {
  const service = host?.agentPresets as
    | { register?: unknown; acquireScope?: unknown; readDocument?: unknown; standingKeyFor?: unknown }
    | undefined;
  ...
  if (declarative && scopeLease) return { declarative, scopeLease, legacyDirectory, reason: '' };
 ...
 */
export function findStandardModeDeclaration(
  loader: ModeLoaderLike | undefined, standardId: string = STANDARD_MODE_ID,
): StandardModeDeclaration | null {
  if (loader === undefined || typeof loader.entries !== 'function') return null;
  for (const entry of loader.entries()) {
    const options = entry?.options;
    if (options === undefined || options.name !== AGENT_PRESET_PLUGIN) continue;
    const config = recordOf(options.config);
    if (config.id !== standardId) continue;
...
export function standardModeDeclaration(
  host: ModeHostLike | undefined, standardId: string = STANDARD_MODE_ID,
): StandardModeDeclaration | null {
  return findStandardModeDeclaration(host?.loader, standardId);
}

/** Options of {@link buildPresetModeDefinition}. */
export interface BuildPresetModeOptions {
  id?: string;
  name?: string;
  description?: string;
  order?: number;
  /** Plugin specifier of the preset-enhance mode marker row. */
  presetEntryName?: string;
  /** Extra rows appended after the marker row. */
  append?: readonly ModePluginRow[];
}
 ...
 */
export function buildPresetModeDefinition(
  standard: ModeDefinition, options: BuildPresetModeOptions = {},
): ModeDefinition {
  const plugins = standard.plugins.map(row => ({ ...row }));
  const personaIndex = plugins.findIndex(row => row.name === PERSONA_PLUGIN);
  if (personaIndex < 0) throw new Error('standard 模式组成中找不到 persona 条目');
  ...
  if (!hasEntry) plugins.push({ id: PRESET_MODE_ENTRY_ID, name: entryName });
  if (Array.isArray(options.append)) {
    for (const row of options.append) if (isPluginRow(row)) plugins.push({ ...row });
  }
  return {
...
/** Bounded wait options for {@link waitForStandardDeclaration}. */
export interface WaitForStandardOptions {
  /** Give up after this many milliseconds; default 5000. */
  timeoutMs?: number;
  /** Re-scan interval; default 50 ms. */
  intervalMs?: number;
}
 ...
 */
export async function waitForStandardDeclaration(
  host: ModeHostLike | undefined, options: WaitForStandardOptions = {},
  standardId: string = STANDARD_MODE_ID,
): Promise<StandardModeDeclaration | null> {
...
/** One live registration returned by {@link createPresetModeController}. */
export interface PresetModeRegistration {
  readonly id: string;
  readonly definition: ModeDefinition;
  readonly standard: StandardModeDeclaration;
  /** Idempotent: unregisters from the host exactly once. */
  dispose(): Promise<void>;
}

/** Options of {@link createPresetModeController}. */
export interface PresetModeControllerOptions extends BuildPresetModeOptions {
  /** Standard declaration id to copy. */
  standardId?: string;
}

/** Default rows mirrored from the 0.1.6 generated mode directory. */
export interface PresetModeController {
  readonly capability: ModeCapability;
  /** The host's current standard declaration, read synchronously from the loader. */
  standard(): StandardModeDeclaration | null;
  /** Register the mode; safe to call repeatedly and concurrently. */
  register(): Promise<PresetModeRegistration>;
  /** Wait (bounded) for a late-composed standard declaration, then register. */
  registerWhenAvailable(wait?: WaitForStandardOptions): Promise<PresetModeRegistration>;
  /** The current registration, or null. */
  current(): PresetModeRegistration | null;
  /** Idempotent teardown; also unregisters a registration still in flight. */
  dispose(): Promise<void>;
}

/** Release one scope lease, tolerating the spellings a host may use. */
export async function releaseModeScope(lease: ModeScopeLease | undefined | null): Promise<void> {
  if (lease == null) return;
  const releasable = lease as {
    [Symbol.asyncDispose]?: () => Promise<void>;
    release?: () => Promise<void>;
    dispose?: () => Promise<void>;
  };
  const release = releasable[Symbol.asyncDispose] ?? releasable.release ?? releasable.dispose;
  if (typeof release !== 'function') throw new Error('模式作用域租约无法释放：宿主未提供异步释放方法');
  await release.call(lease);
}

function rowsOf(value: unknown): ToolSchemaRow[] {
  return Array.isArray(value) ? value.filter(entry => typeof recordOf(entry).name === 'string') as ToolSchemaRow[] : [];
}
 ...
 */
export function createPresetModeController(
  host: ModeHostLike | undefined, options: PresetModeControllerOptions = {},
): PresetModeController {
  const capability = modeCapability(host);
  const standardId = options.standardId ?? STANDARD_MODE_ID;
  let active: PresetModeRegistration | null = null;
  let inflight: Promise<PresetModeRegistration> | null = null;
  let epoch = 0;

  async function performRegistration(): Promise<PresetModeRegistration> {
    if (!capability.declarative) throw new Error(capability.reason);
    ...
    if (typeof register !== 'function') throw new Error(capability.reason);
  ...
  function registerMode(): Promise<PresetModeRegistration> {
    if (active !== null) return Promise.resolve(active);
    if (inflight !== null) return inflight;
    const started = epoch;
    const run = (async (): Promise<PresetModeRegistration> => {
  ...
  return {
    ...
    async registerWhenAvailable(wait: WaitForStandardOptions = {}): Promise<PresetModeRegistration> {
    ...
    async dispose(): Promise<void> {
      ...
      if (pending !== null) { try { await pending; } catch { /* the pending registration released itself */ } }
...
/** One mode's tool catalog, read through a lease that was released again. */
export interface ModeToolCatalog {
  readonly modeId: string;
  /** The scope key used for the read; its identity is the mode's generation. */
  readonly scope: unknown;
  readonly tools: ToolSchemaRow[];
}
 ...
 */
export async function readModeToolCatalog(host: ModeHostLike | undefined, modeId: string): Promise<ModeToolCatalog> {
...
/** Where one session's tool list came from. */
export type SessionCatalogSource = 'session' | 'mode' | 'none';

/** One session's tool list. */
export interface SessionToolCatalog {
  readonly sessionId: string;
  /**
   * `session`: read from the live session's own scope, i.e. the exact revision
   * it joined. `mode`: a cold-session fallback to the mode's CURRENT revision
   * (never proof of what that session used). `none`: no live agent and no
   * fallback requested.
   */
  readonly source: SessionCatalogSource;
  /** Present only for `mode`; the current generation's scope key. */
  readonly scope?: unknown;
  readonly tools: ToolSchemaRow[];
  /** Set when a read was attempted and failed; `tools` is then empty. */
  readonly error?: string;
}

/** Options of {@link readSessionToolCatalog}. */
export interface SessionCatalogOptions {
  /**
   * Mode to fall back to when the session has no live agent. The result is
   * marked `source: 'mode'` because an old session may hold another revision.
   */
  modeId?: string;
}
 ...
 * session's tool set.
 */
export async function readSessionToolCatalog(
  host: ModeHostLike | undefined, sessionId: string, options: SessionCatalogOptions = {},
