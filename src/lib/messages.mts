...
import type { ContentBlock, HostMessage } from '../host-types.mjs';

/** Protocol switch persisted with the preset settings. */
export type PresetProtocol = 'chat-completions' | 'messages';

/** One best-effort adaptation result. */
export interface MessagesAdaptation {
  /** A new array; unchanged messages keep their original object identity. */
  messages: HostMessage[];
  /** User-facing Chinese notes, one per distinct limitation, without duplicates. */
  notes: string[];
  /**
   * How many leading `system` messages were merged into the single leading one;
   * 0 when no merge was needed (fewer than two leading system messages).
   */
  leadingSystemMerged: number;
  /** True when the trailing assistant message is a continuation prefix. */
...
function isSystemMessage(message: HostMessage | undefined): boolean {
 ...
 */
export function adaptPresetForMessages(messages: HostMessage[]): MessagesAdaptation {
  const notes: string[] = [];
  const note = (text: string): void => {
    if (!notes.includes(text)) notes.push(text);
  };
  if (!Array.isArray(messages)) return { messages: [], notes, leadingSystemMerged: 0, prefixUnsupported: false };

  const out = messages.slice();
  let leading = 0;
  while (leading < messages.length && isSystemMessage(messages[leading])) leading += 1;
  ...
  if (leading >= 2) {
    ...
    const content: ContentBlock[] = [{ type: 'text', text: mergedText }];
  ...
  if (midHistory > 0) note(midHistoryNote(midHistory));

  const prefixUnsupported = isPrefixMessage(messages.at(-1));
  if (prefixUnsupported) note(PREFIX_UNSUPPORTED_NOTE);
