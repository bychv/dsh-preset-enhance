...
- “管理分组”可新增、重命名、排序、删除分组，并用复选框分配工具；同一个工具最多属于一个用户分组，未分组的工具显示在“未分组”标签。标签栏和内容区的折叠状态只保存在浏览器 localStorage，不进入分享文件。
...
工作台支持导入和导出单个 `.dsh-preset.json` 文件，包含完整 ST 预设、预填充接口配置以及工具预设和工具分组。导出时默认只带当前关联的工具预设及其分组，不包含模式默认/会话选择、会话 ID、目录缓存、引用计数或折叠状态。导入文件本身不会改动接口设置或工具开关：包内接口设置需手动“应用包内接口设置（全局）”，包内工具配置需手动“导入包内工具配置”，应用前会显示新增、复用、重映射和匹配、失配统计。未知的工具子版本仍可导入、编辑和再次导出，但禁止应用。也可用“导出 ST JSON”生成普通预设文件。格式与字段约定见 [单文件格式说明](PRESET_FORMAT.md)。
