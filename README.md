...
当前分支声明的宿主范围为 `>=0.1.5-rc.2 <0.1.6` 与 `0.1.6-alpha.2`。后续 0.1.6 版本需重新验证，不提前声明兼容整个系列。

## 连接协议与能力范围

DSH 0.1.6 的官方连接默认使用 Messages。工作台按当前会话实际使用的连接读取最新协议；顶部的协议滑块直接修改该连接的宿主设置，切换后无需重启。它是连接级设置，会影响使用同一连接的其他会话。
...
预设库支持导入、编辑、预览、导出和删除。最后选择的预设全局保存，重启后继续作为默认预设使用。
...
“预设模式”从宿主的 `standard` 模式生成，继承标准工具集，并移除 DSH 身份提示与运行环境快照。其他模式叠加预设时，由置顶的只读“DSH 系统提示词”条目控制是否保留模式原有提示；旧预设默认保留。
