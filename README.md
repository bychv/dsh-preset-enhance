...
预设里若带有酒馆格式的正则规则，工作台会多出一个可折叠的「提示词正则」分区。

- 打开总开关后，规则作用于**发送给模型的内容**：可以清理历史正文、按聊天深度保留不同内容。默认关闭。
- 「同时处理预填充」决定是否也处理最终追加的助手续写前缀，默认关闭。
- 规则只改发送内容，**不会改变聊天里的显示**；已经发生的历史与原始预设都不会被改写。
- 本版本只执行「仅格式化提示词」且作用位置为「用户输入」或「助手输出」的规则。显示侧、斜杠命令、世界书与推理位置的规则会保留并说明原因，但不会执行。
- 测试框可以输入文本、选择作用对象与深度，查看替换前后与命中的规则；它与真正发送时使用同一套处理。
### 保存
