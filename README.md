...
DSH 0.1.6 的官方连接默认改用 Messages 协议。插件会按当前会话实际使用的连接读取最新协议，并在工作台顶部提供该连接的协议开关：切换后立即生效，不需要重启，也不需要改配置文件。

- **对话补全接口**：预填充续写、工具调用转换与正文提取都正常工作。
- **Messages 接口**：仅提供有限的预设注入兼容，预填充续写、工具调用转换与正文提取不会应用。历史中段或末尾的 system 提示可能被重排或覆盖；需要准确保持预设顺序时请使用对话补全接口。插件会把预设开头的多条 system 提示合并后发送，避免其中内容被丢弃；工作台会禁用“Assistant 预填充接口”设置；切回对话补全接口后恢复可配置。
...
- **正文/工具调用提取（实验）**：按“独占一行的结束思考标记 + 10 token 窗口 + 输出区标签兜底”的最终稳定性策略处理流式和非流式响应。切换前文本进入 `reasoning_content`，输出区进入正文，输出区控制标签会被移除，随后继续执行 DSML 工具调用恢复。启用时自动将完整格式提示词加入当前预设的 `chatHistory` 后方、assistant 预填充前；可在提示词列表中编辑，重复开启会保留已有编辑。保存接口设置时会一并保存预设；该功能默认关闭。
...
- 开启后，逐工具复选框、全选/全不选、分组三态开关以及“全开”“全关”“仅启用此组”“恢复预设值”等**工具开关**改动会在约 400 ms 防抖后立即保存（模式默认写入模式，会话覆盖写入当前会话），一次连续编辑只发一个请求，状态栏显示“已自动保存（范围 · 模式）”；页面隐藏或关闭时会尽力再冲刷一次尚未发送的改动，并在本地暂存未确认的工具草稿，刷新或重新打开相同范围后继续保存。
